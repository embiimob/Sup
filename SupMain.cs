                        ...
                        case "IPFS":
                            ...
                            if (!System.IO.Directory.Exists("ipfs/" + transid + "-build"))
                            {
                                Task.Run(async () =>
                                {
                                    ...
                                    // Use IpfsHelper for async, timeout-protected IPFS operations
                                    bool success = await IpfsHelper.GetAsync(transid, "ipfs/" + transid, 60000).ConfigureAwait(false);
                                    
                                    if (success)
                                    {
                                        // Determine target file name from full IPFS path
                                        string fileName = activeProfile.Image.Replace(@"//", "").Replace(@"\\", "").Substring(51);
                                        if (fileName == "") { fileName = "artifact"; } 
                                        else { fileName = fileName.Replace(@"/", "").Replace(@"\", ""); }
                                        
                                        // Process the downloaded file
                                        if (activeProfile.Image.LastIndexOf('.') > 0 && fileName == "artifact")
                                        {
                                            fileName = "artifact" + activeProfile.Image.Substring(activeProfile.Image.LastIndexOf('.'));
                                        }
                                        
                                        IpfsHelper.ProcessDownloadedFile(transid, "ipfs", fileName);
                                        
                                        // Update final image path
                                        if (File.Exists("ipfs/" + transid + "/" + fileName))
                                        {
                                            imgurn = "ipfs/" + transid + "/" + fileName;
                                        }
                                        else if (activeProfile.Image.LastIndexOf('.') > 0)
                                        {
                                ...
                                else
                                {
                                    ...
                                    switch (objstate.Image.ToUpper().Substring(0, 4))
                                    {
                                        ...
                                        case "IPFS":
                                            ...
                                            if (!System.IO.Directory.Exists("ipfs/" + transid + "-build"))
                                            {
                                                Task.Run(async () =>
                                                {
                                                    ...
                                                    if (success)
                                                    {
                                                        ...
                                                        // Process the downloaded file
                                                        if (objstate.Image.LastIndexOf('.') > 0 && fileName == "artifact")
                                                        {
                                                            fileName = "artifact" + objstate.Image.Substring(objstate.Image.LastIndexOf('.'));
                                                        }
                                                        ...
                                                        else if (objstate.Image.LastIndexOf('.') > 0)
                                                        {
                                            ...
                                            {
                                                // Build directory exists, wait for another task to complete the download
                                                Task.Run(async () =>
                                                {
                                                    // Wait up to 10 seconds for file to appear (another task is downloading)
                                                    for (int i = 0; i < 100; i++)
                                                    {
                                                        if (System.IO.File.Exists(imgurn))
                                                        {
                                            ...
                                            case "IPFS":
                                                ...
                                                if (!System.IO.Directory.Exists("ipfs/" + transid + "-build"))
                                                {
                                                    Task.Run(async () =>
                                                    {
                                                        ...
                                                        // Use IpfsHelper for async, timeout-protected IPFS operations (60s timeout)
                                                        bool success = await IpfsHelper.GetAsync(transid, "ipfs/" + transid, 60000).ConfigureAwait(false);
                                                        
                                                        if (success)
                                                        {
                                                            // Determine target file name
                                                            string fileName = profile.Image.Replace(@"//", "").Replace(@"\\", "").Substring(51);
                                                            if (fileName == "") { fileName = "artifact"; } 
                                                            else { fileName = fileName.Replace(@"/", "").Replace(@"\", ""); }
                                ...
                                case "IPFS":
                                    ...
                                    if (!System.IO.Directory.Exists("ipfs/" + transid + "-build"))
                                    {
                                        Task.Run(async () =>
                                        {
                                            ...
                                            {
                                                // Determine target file name
                                                string fileName = imagepath.Replace(@"//", "").Replace(@"\\", "").Substring(51);
                                                if (fileName == "") { fileName = "artifact"; } 
                                                else { fileName = fileName.Replace(@"/", "").Replace(@"\", ""); }
                                ...
                                case "IPFS":
                                    ...
                                    if (!System.IO.Directory.Exists("ipfs/" + transid + "-build"))
                                    {
                                        Task.Run(async () =>
                                        {
                                            ...
                                            {
                                                // Determine target file name
                                                string fileName = videopath.Replace(@"//", "").Replace(@"\\", "").Substring(51);
                                                if (fileName == "") { fileName = "artifact"; } 
                                                else { fileName = fileName.Replace(@"/", "").Replace(@"\", ""); }
