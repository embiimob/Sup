...
namespace SUP
{
    public partial class SupMain : Form
    {
        ...
        private static bool IsImageExtension(string extension) => CommonImageExtensions.Contains(extension);
        ...
        private static bool IsMediaExtension(string extension) =>
            IsImageExtension(extension) || IsVideoExtension(extension) || IsAudioExtension(extension);
        ...
        private void MakeActiveProfile(string address)
        {
            ...
            if (activeProfile.Image != null)
            {
                ...
                if (!activeProfile.Image.ToLower().StartsWith("http"))
                {
                    ...
                    if (activeProfile.Image.ToLower().StartsWith("ipfs:")) { imgurn = imgurn.Replace(@"\root\", @"\ipfs\"); if (activeProfile.Image.Length == 51) { imgurn += @"\artifact"; } else if (activeProfile.Image.Length > 51 && activeProfile.Image[51] == '.') { string _ext = activeProfile.Image.Substring(51); imgurn = imgurn.Substring(0, imgurn.Length - _ext.Length) + @"\artifact" + _ext; } }
                }
                            ...
                            if (objstate.Image != null)
                            {
                                ...
                                if (!imgurn.Contains(@"root\keywords\"))
                                {
                                    if (!objstate.Image.ToLower().StartsWith("http"))
                                    {
                                        ...
                                        if (objstate.Image.ToLower().StartsWith("ipfs:")) { imgurn = imgurn.Replace(@"\root\", @"\ipfs\"); if (objstate.Image.Length == 51) { imgurn += @"\artifact"; } else if (objstate.Image.Length > 51 && objstate.Image[51] == '.') { string _ext = objstate.Image.Substring(51); imgurn = imgurn.Substring(0, imgurn.Length - _ext.Length) + @"\artifact" + _ext; } }
                                    }
                                }
                                Regex regexTransactionId = new Regex(@"\b[0-9a-f]{64}\b");
                                Match imgurnmatch = regexTransactionId.Match(imgurn);
                ...
                try
                {
                        ...
                        if (profile.URN != null)
                        {
                            ...
                            if (profile.Image != null && !profile.Image.ToLower().StartsWith("http"))
                            {
                                ...
                                if (profile.Image.ToLower().StartsWith("ipfs:"))
                                {
                                    ...
                                    else if (profile.Image.Length > 51 && profile.Image[51] == '.') { string _ext = profile.Image.Substring(51); imagelocation = imagelocation.Substring(0, imagelocation.Length - _ext.Length) + @"\artifact" + _ext; }
                                }
                            }
                        }
                        ...
                        (blocks.Length == 1 && !int.TryParse(blocks[0], NumberStyles.Any, CultureInfo.GetCultureInfo("en-US"), out _)))
                    {
                        ...
                        foreach (string block in blocks)
                        {
                            ...
                            {
                                string extension = GetNormalizedExtension(block);
                                attachment.Extension = extension;
                                
                                HashSet<string> imageExts = new HashSet<string>(CommonImageExtensions, StringComparer.OrdinalIgnoreCase);
                                HashSet<string> videoExts = new HashSet<string>(CommonVideoExtensions, StringComparer.OrdinalIgnoreCase);
                                HashSet<string> audioExts = new HashSet<string>(CommonAudioExtensions, StringComparer.OrdinalIgnoreCase);
            ...
            try
            {
                ...
                foreach (var attachment in viewModel.Attachments)
                {
                    ...
                    else if (attachment.Type == AttachmentType.Image)
                    {
                        this.Invoke((MethodInvoker)delegate
                        {
                            if (IsSvgExtension(attachment.Extension))
                            {
        ...
        private async Task DisplaySecAttachmentAsync(string transid, string recipientAddress)
        {
            try
            {
                ...
                if (decryptedroot.File != null)
                {
                    ...
                    {
                        string extension = GetNormalizedExtension(transid + @"\" + file);
                        ...
                        this.Invoke((MethodInvoker)delegate
                        {
                            ...
                            else if (IsVideoExtension(extension) || IsAudioExtension(extension) || IsSvgExtension(extension))
                            {
                        ...
                        else
                        {
                            ...
                            if (!Path.GetExtension(imagelocation).Equals(".gif", StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                Task.Run(() =>
                                {
                                    try
                                    {
                                        if (IsUnsupportedImageFormat(imgLoc)) return;
