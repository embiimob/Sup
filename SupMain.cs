                            ...
                            if (!int.TryParse(content, NumberStyles.Any, CultureInfo.GetCultureInfo("en-US"), out int id) && !content.Trim().StartsWith("#"))
                            {
                                ...
                                if (content.StartsWith("IPFS:") && content.EndsWith(@"\SEC"))
                                {
                                    ...
                                    if (!System.IO.Directory.Exists("root/" + transid + "-build"))
                                    {
                                        ...
                                        process2.WaitForExit();
                                        if (process2.ExitCode == 0 || System.IO.File.Exists("root/" + transid) || System.IO.File.Exists("root/" + transid + "/" + transid))
                                        {
                                            string fileName;
                                        ...
                                        else
                                        {
                                            ...
                                            Task.Run(() =>
                                            {
                                                ...
                                                process2.StartInfo.RedirectStandardOutput = true;
                                                process2.Start();
                                                string asyncOutput = process2.StandardOutput.ReadToEnd();
                                                process2.WaitForExit();
                                                if (process2.ExitCode == 0 || System.IO.File.Exists("root/" + transid) || System.IO.File.Exists("root/" + transid + "/" + transid))
                                                {
                                                    string fileName;
